...
exports.winnersAnnouncement = exports.exportPaymentWithdraw = exports.getPaymentWithdraw = void 0;
...
const getPaymentWithdraw = async (req, res) => {
    try {
        ...
        shippingaddressCount = await PaymentWithdraw_1.default.count({
            where: searchCondition,
        });
        shippingaddress = await PaymentWithdraw_1.default.findAll({
            where: searchCondition,
            attributes: ["id", "full_name", "phone_number", "payment_gateway", "payment_number", "upi_id", "paypal_id", "crypto_id", "is_winner"],
            limit: limit,
            offset: offset,
            raw: true,
        });
...
exports.getPaymentWithdraw = getPaymentWithdraw;
const exportPaymentWithdraw = async (req, res) => {
    try {
        let { query } = req, expotFiled;
        const publicDir = path_1.default.join(__dirname, '../../public/uploads/exports_file');
        let shippingaddress = await PaymentWithdraw_1.default.findAll({
...
exports.exportPaymentWithdraw = exportPaymentWithdraw;
const winnersAnnouncement = async (req, res) => {
    try {
        let winRes;
        let { query, body } = req;
        if (parseInt(query.winner_id)) {
            winRes = await PaymentWithdraw_1.default.update(body, {
                where: {
                    id: parseInt(query.winner_id)
                }
            });
        }
        else {
